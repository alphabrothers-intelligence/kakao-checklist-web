...
우리채널 알리기|친구 추가할 때만 비용 발생, 부담 없이 시작해요
채널 메시지|쿠폰 소식 보내 다시 찾아오게 만들어요`,
`우리매장 맵광고|지도 검색만으로 신규 손님을 만나요
우리채널 알리기|시술 손님을 채널 친구로 빠르게 모아요
채널 메시지|시술 주기에 맞춰 재방문을 리마인드해요`,
`카카오비즈보드|연령·지역 필터링으로 폭넓게 노출돼요
비즈니스폼|톡 안에서 바로 상담 신청을 받아요
키워드광고|찾는 손님에게만 노출, 예산 낭비가 적어요`,
`카카오비즈보드|신상품을 넓은 타겟에 빠르게 알려요
카카오모먼트 디스플레이|장바구니 놓친 손님을 다시 데려와요
채널 메시지|할인 소식으로 구매를 마무리 지어요`,
`우리매장 맵광고|지도에서 근처 손님 눈에 바로 띄어요
우리채널 알리기|적은 비용으로 단골 채널을 모아요
키워드광고|찾는 손님에게 노출, 방문 전환율이 높아요`,
`카카오비즈보드|학부모·학생층에 폭넓게 알려요
비즈니스폼|상담·설명회 신청을 놓치지 않아요
채널 메시지|개강·모집 소식과 재등록을 챙겨요`,
`키워드광고|견적·상담을 찾는 손님에게 바로 노출돼요
비즈니스폼|전화 없이도 견적 문의를 받아요
우리매장 맵광고|동네 기반 업체는 지도 노출로 추가 유입돼요`
].map(s=>s.split('\n').map(row=>row.split('|')));
export const advice = [
`유입 부족
사이트 앞까지 손님이 오지 않고 있어요. 검색·온라인 채널 노출부터 넓혀보세요.
광고를 누르고 들어온 페이지에 원하는 내용이 바로 보이게 하세요.
노출만 늘리면 소용없어요. 로딩 속도와 모바일 화면부터 확인하세요.`,
`유입 부족
근처 손님도 우리 매장을 모르고 있어요. 지도·검색에 위치, 사진, 영업시간부터 정확히 올리세요.
찾아온 손님이 헤매지 않게 오시는 길과 예약 방법을 눈에 띄게 정리하세요.
노출만 늘리면 소용없어요. 매장 사진과 후기부터 매력적인지 확인하세요.`,
`관심 부족
광고는 보는데 반응이 없어요. 문구와 이미지를 손님 입장에서 다시 써보세요.
타겟팅을 관심사·연령으로 좁혀서, 반응할 손님에게만 보여주세요.
소재 여러 개를 동시에 테스트해서 반응 좋은 것만 남기세요.`,
`전환 부족
관심은 있는데 마지막 한 걸음을 안 떼고 있어요. 구매·예약 버튼 위치부터 확인하세요.
상담·예약형이면 문의 창구를 한 번에, 구매형이면 결제 단계를 최대한 줄이세요.
가격이나 절차가 부담스럽진 않은지 점검하고, 첫 구매·예약엔 혜택을 얹어주세요.`,
`재구매 부족
한 번 오고 다시 안 와요. 다시 연락할 수 있는 채널부터 만들어두세요.
채널 친구를 모아두고, 재방문 시점에 맞춰 쿠폰·리마인드를 보내세요.
메시지를 너무 자주 보내면 오히려 떠나요. 꼭 필요할 때만 보내세요.`,
`측정 부족
광고 효과가 눈에 안 보여요. 전환 추적 코드부터 설치하고 CPA 기준을 정하세요.
어떤 채널·소재가 실제 구매·문의로 이어졌는지 추적해서, 성과 낮은 곳은 예산을 줄이세요.
예산을 한 번에 쓰지 마세요. 소액으로 반응부터 확인하고 늘려가세요.`,
`측정 부족
광고 보고 몇 명이 왔는지 알 수가 없어요. 쿠폰 사용, 전화 문의, 예약 건수처럼 셀 수 있는 지표부터 만드세요.
소재에 "이 쿠폰을 보여주세요" 같은 확인 장치를 넣어서, 광고로 온 손님을 구분하세요.
정확한 숫자보다 광고 전후로 방문·문의가 늘었는지 흐름을 보세요.`
].map(s=>s.split('\n'));
export const pairs = [
