...
export const Result = forwardRef<HTMLDivElement, { result: Diagnosis; answers: CompleteAnswers; onEdit: () => void; onRestart: () => void; onSave: () => void; onGuide: () => void }>(function Result({ result, answers, onEdit, onRestart, onSave, onGuide }, ref) {
  return <div className="container result" ref={ref}>
    ...
    <div className="chips">{[result.industry, result.path, result.situation, result.goal].map(v => <span className="chip" key={v}>{v}</span>)}<button className="textbtn edit-answers" data-export-ignore onClick={onEdit}>답변 수정</button></div>
    ...
    <section className="rsection" aria-labelledby="advice-title">
      ...
      {result.advice.map(({ key, content, context }) => <div className="advice" key={key}>
        {context && <div className="advice-context"><Emoji name={key.endsWith('online') ? 'cart' : 'store'} /><span>{context}</span></div>}
        <h3><span className="advice-caption">발견된 병목</span><span className="problem-icon"><Emoji name={OPTION_ICONS[2][answers[2]]} /></span><span className="problem-name">{content[0]}</span></h3>
        {content.slice(1).map((text, i) => <div className="advice-row" key={i}><h4><Emoji name={['idea', 'tools', 'notice'][i]} /><span>{['상황 해결', '구조 개선', '주의 사항'][i]}</span></h4><p>{text}</p></div>)}
      </div>)}
    </section>
