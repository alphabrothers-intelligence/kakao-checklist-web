...
| 결과별 광고 가이드 링크 20개 | `src/data/source.json`의 `GUIDES` |
| 스타터 키트 링크 | `src/components/Dialogs.tsx`의 `STARTER_KIT_URL` |
| 링크 공유 썸네일·제목·설명 | `index.html`의 og/twitter 메타, `public/og-image.png` |
...
- 현재 문제는 PC 15px·모바일 14px, 상황 해결·구조 개선·주의 사항은 17px·16px Regular로 표시합니다. 네 항목 모두 옅은 잉크 색을 유지합니다. 문제 아이콘과 문장은 분리된 열로 정렬합니다.
...
- 가이드 창의 두 버튼은 실제 노션 문서로 연결되어 있습니다. 더 이상 ‘준비 중’ 비활성 버튼이 아닙니다.
- 가이드 링크는 `GUIDES[Q2 경로][Q4 목표]`로 고릅니다. 업종과 문제는 관여하지 않습니다. 이 축을 바꾸면 20개 링크 배열의 모양도 함께 바꿔야 합니다.
- `public/og-image.png`는 인트로 디자인을 본떠 만든 정적 이미지입니다. 인트로 문구나 캐릭터가 바뀌어도 자동으로 따라가지 않으므로 직접 다시 만들어야 합니다.
